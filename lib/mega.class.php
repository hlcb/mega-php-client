...
class MEGA {
     ...
     */
    public function public_file_info( $ph, $key = null, $dl_url = FALSE, $args = array() ) {

        /**
         * Accepts arguments in different orders
         * 
         * ph + key
         * link 73
         * link 70
        ...
        if ( !is_string( $ph ) || !is_string( $key ) ) {
            return false;
        }

        if ( strlen( $ph ) !== 8 || strlen( $key ) !== 43 ) {
            return false;
        }
     ...
     */
    public function public_file_download( $ph, $key = null, $dest = null ) {
        ...
        if ( is_string( $_ph ) ) {
            ...
            if ( strlen( $_ph ) === 73 ) {
                ...
                if ( strlen( $_key ) === 73 ) {
                    ...
                    if ( is_writable( dirname( $_ph ) ) ) {

                        $dest = $_ph;

                    } elseif ( is_resource( $_ph ) ) {
        ...
        } elseif ( is_string( $dest ) && is_writable( dirname( $dest ) ) ) {
        ...
        if ( is_null( $dest ) || is_resource( $dest ) ) {
            rewind( $handle );
     ...
     */
    public function public_file_save( $ph, $key = null, $dir_path = null, $filename = null ) {
        ...
        if ( is_string( $_key ) && ( substr( $_key, 0, 1 ) === '/' || substr( $_key, 1, 1 ) === ':' ) ) {
            if ( ( is_dir( dirname( $_key ) ) || mkdir( dirname( $_key ), 0755, true ) ) && !is_dir( $dir_path ) ) {
                $dir_path = $_key;
            }
        }
        ...
        if ( !$info || empty( $info['g'] ) ) {
            if ( !empty( $info['e'] ) && ( $error_message = MEGAException::getMessageFromCode( $info['e'] ) ) ) {
    ...
    public function user_login_session( $email, $password, $args = array() ) {
        ...
        if ( strpos( $password, '@' ) !== false && strpos( $email, '@' ) === false ) {
            $email = $_password;
            $password = $_email;
        }
        ...
        /*
        #$password_aes = MEGACrypto::prepare_key( MEGAUtil::str_to_a32( $password ) );
        #$enc_master_key = MEGAUtil::base64_to_a32( $res['k'] );
        #$master_key = MEGACrypto::decrypt_key( $password_aes, $enc_master_key );
        //*/
        ...
        if (isset($res['k'])) {
            ...
            if ( count( $k ) === 4 ) {
                ...
                if ( isset( $res['tsid'] ) ) {
                    $tsid = MEGAUtil::base64urldecode( $res['tsid'] );
                    if ( MEGAUtil::a32_to_str( MEGACrypto::encrypt_key( MEGAUtil::a32_to_str( $k ), MEGAUtil::str_to_a32( substr( $tsid, 0, 16 ) ) ) ) == substr( $tsid, -16 ) ) {
                        $sid = $res['tsid'];
                    }
                    ...
                    for ( $i = 0; $i < 4; $i++ ) {
                        ...
                        #$l = ( ( MEGAUtil::charCodeAt( $privk[0] ) * 256 + MEGAUtil::charCodeAt( $privk[1] ) + 7 ) >> 3 ) + 2;
                        #$l = ( ( MEGAUtil::charCodeAt( $privk[0] ) * 256 + MEGAUtil::charCodeAt( $privk[1] ) + 7 ) >> 3 );
                        //*/
    ...
    public function user_add($args = array()) {
        ...
        for ( $i = 4; $i--; ) {
            $password_key[$i] = rand( 0, PHP_INT_MAX );
        }

        for ( $i = 4; $i--; ) {
            $u_k[$i] = rand( 0, PHP_INT_MAX );
        }

        for ( $i = 4; $i--; ) {
            $ssc[$i] = rand( 0, PHP_INT_MAX );
        }

        $req_a = 'up';
        $req_k = MEGAUtil::a32_to_base64( MEGACrypto::encrypt_key( MEGAUtil::a32_to_str( $password_key ), $u_k ) );
        $req_ts = MEGAUtil::base64urlencode( MEGAUtil::a32_to_str( $ssc) . MEGAUtil::a32_to_str( MEGACrypto::encrypt_key( MEGAUtil::a32_to_str( $u_k ), $ssc ) ) );
    ...
    protected function http_do_request( $url, $payload ) {
        ...
        /*
        #MEGACache::save_cache_file( __METHOD__, $_file, $content );
        //*/
...
class MEGAException extends Exception {
    ...
    public static function getMessageFromCode( $code = null ) {
        if ( is_array( $code ) && !empty( $code['e'] ) ) {
            $code = $code['e'];
        }
        $message = NULL;
        switch ( $code ) {
...
class MEGACrypto {
    ...
    public static function crypto_encodeprivkey( $privk ) {
        $plen = strlen( $privk[3] ) * 8;
        $qlen = strlen( $privk[4] ) * 8;
        $dlen = strlen( $privk[2] ) * 8;
        $ulen = strlen( $privk[7] ) * 8;

        $t = MEGAUtil::fromCharCode( $qlen / 256) . MEGAUtil::fromCharCode( $qlen % 256) . $privk[4]
            . MEGAUtil::fromCharCode( $plen / 256) . MEGAUtil::fromCharCode( $plen % 256) . $privk[3]
            . MEGAUtil::fromCharCode( $dlen / 256) . MEGAUtil::fromCharCode( $dlen % 256) . $privk[2]
            . MEGAUtil::fromCharCode( $ulen / 256) . MEGAUtil::fromCharCode( $ulen % 256) . $privk[7];

        while ( strlen( $t ) & 15 ) $t .= MEGAUtil::fromCharCode( rand( 0, 256 ) );

        return $t;

    }

    public static function crypto_encodepubkey( $pubkey ) {

        $mlen = strlen( $pubkey[0] ) * 8;
        $elen = strlen( $pubkey[1] ) * 8;

        return MEGAUtil::fromCharCode( $mlen / 256 ) . MEGAUtil::fromCharCode( $mlen % 256 ) . $pubkey[0]
            . MEGAUtil::fromCharCode( $elen / 256 ) . MEGAUtil::fromCharCode( $elen % 256 ) . $pubkey[1];
...
class MEGARsa {
    ...
    public static function int2bin( $num ) {
        ...
        if ( MEGACache::has_cache_file( __METHOD__, $num, $_file ) ) {
            return MEGACache::load_cache_file( __METHOD__, $_file );
        }
        ...
        MEGACache::save_cache_file( __METHOD__, $_file, $result );
        return $result;
    }
...
class MEGAUtil {
    ...
    public static function str_to_a32( $b ) {
        ...
        $_result = array_values( unpack( 'N*', $b ) );
        $_static[$_b] = $_result;
        return $_result;
    }
    ...
    public static function str_depad($b) {
        for ($i = strlen($b); $i-- && !self::charCodeAt($b[$i]); );
        $b = substr($b, 0, $i + 1);
        return $b;
    }
    ...
    public static function mpi2b( $s ) {
        if ( MEGACache::has_cache_file( __METHOD__, $s, $_file, $_ref ) ) {
            return MEGACache::load_cache_file( __METHOD__, $_file );
        }
        $s = substr( $_ref, 4 );
        $len = strlen( $s );
        ...
        MEGACache::save_cache_file( __METHOD__, $_file, $n );
        return $n;
    }
    ...
    */
    ...
    public static function fromCharCode( $num ) {
...
class MEGACache {
    ...
    public static function getDocumentRoot() {
        ...
        } elseif ( basename( __DIR__ ) === 'classes' ) {
            $root = realpath( __DIR__ . '/../' ) . DIRECTORY_SEPARATOR;
        } elseif ( basename( dirname( __DIR__ ) ) === 'classes' ) {
            $root = realpath( __DIR__ . '/../../' ) . DIRECTORY_SEPARATOR;
        } else {
    ...
    public static function get_cache_folder( $method = null ) {
        ...
        $folder = self::getDocumentRoot() . '/cache/classes/MEGA/MEGA/cache/';
        ...
        if ( count( $parts ) === 2 ) {
            return sprintf( self::getDocumentRoot() . '/cache/classes/MEGA/%s/%s/', $parts[0], $parts[1] );
        }
    ...
    public static function get_cache_filename( $method = null, $vars = array(), &$ref = null ) {

        if ( isset( $vars['_folder'] ) ) {
            unset( $vars['_folder'] );
        }
        ...
        switch ( $method ) {
            case 'MEGACrypto::prepare_key':
                ...
                if ( isset( $vars['a'] ) ) {
                    $filename = implode( '-', $vars['a'] );
                }
                break;
            case 'MEGACrypto::stringhash':
                /*
                #$vars['s'] . '-' . bin2hex( $vars['aeskey'] )
                #$vars['s']
                #md5( serialize( $vars ) )
                //*/
                $filename = $vars['s'];
                break;
            case 'MEGA::http_do_request':
                $vars = isset( $vars['payload'] ) ? $vars['payload'] : $vars;
                $filename = md5( $vars );
            case 'MEGAUtil::mpi2b':
                $vars = isset( $vars['s'] ) ? $vars['s'] : $vars;
                $ref = bin2hex( $vars );
                $filename = md5( $ref );
                break;
            case 'MEGARsa::int2bin':
                $vars = isset( $vars['num'] ) ? $vars['num'] : $vars;
                $filename = md5( $vars );
                break;
        }

        $filename = isset( $filename ) ? $filename : md5( serialize( $vars ) );
    ...
    public static function has_cache_file( $method = null, $vars = array(), &$file = null, &$ref = null ) {

        $_folder = MEGACache::get_cache_folder( $method );
        $_filename = MEGACache::get_cache_filename( $method, $vars, $ref );
